...
/* ---------- Experience + Education en columnas paralelas ---------- */
(function sideBySideLayout() {
  const exp = document.getElementById('experience');
  const edu = document.getElementById('education');
  if (!exp || !edu || !exp.parentNode) return;

  const wrapper = document.createElement('div');
  wrapper.className = 'exp-edu-columns';
  exp.parentNode.insertBefore(wrapper, exp);
  wrapper.appendChild(exp);
  wrapper.appendChild(edu);
})();
